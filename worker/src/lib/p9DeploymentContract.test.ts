...
assert(!/^ray(?:\[.*\])?\s*[=<>~!]/m.test(mlRequirements), 'ML runtime must not ship unused Ray auto-tuning and its shaded Java stack')
assert(mlDockerfile.includes("assert importlib.util.find_spec('ray') is None"), 'ML image build must fail closed if Ray is reintroduced transitively')
