...
MODEL_SCORE_LINEAGE_SCHEMA_VERSION = "active8-model-score-lineage-v1"
...
def normalize_active8_cross_sectional_scores(
    ...
    *,
    ...
    for symbol, prediction in predictions.items():
        ...
        missing_versions = [name for name in ACTIVE_ALPHA_MODELS if name not in normalized_versions]
        ...
        row_blockers = [f"rank_missing:{name}" for name in missing_scores]
        ...
        signature = build_model_set_signature(normalized_versions, list(ACTIVE_ALPHA_MODELS))
        ...
        prediction["model_score_lineage"] = {
            ...
            "artifact_target_semantics": normalized_target_semantics,
