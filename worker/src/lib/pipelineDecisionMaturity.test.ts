...
test('pipeline maturity API preserves canonical lineage and explicit evidence fields', () => {
  ...
  assert.match(source, /L4_ALPHA_EV_CONTRACT\.artifactContractVersion/)
  assert.match(source, /ALLOCATOR_EV_FUSION_CONTRACT\.artifactContractVersion/)
  assert.match(source, /ORDER BY source_run_date DESC, updated_at DESC, artifact_id DESC/)
  assert.match(source, /version LIKE 'l4-alpha-ev-ridge-v5-%'/)
  assert.match(source, /version LIKE 'allocator-ev-fusion-residual-v14-%'/)
  assert.match(source, /comparison_contract/)
  assert.match(source, /updated_at: l4Shadow\?\.updated_at/)
  assert.match(source, /updated_at: fusionShadow\?\.updated_at/)
  assert.doesNotMatch(source, /fusion_oof_max_date/)
  assert.match(frontendView, /同 contract offline history/)
  assert.match(frontendView, /尚無同 contract 前一筆/)
