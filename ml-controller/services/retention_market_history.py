...
def archived_market_projection(table, sql, params, start_date, end_date, *, query_hot=None, query_ops=None, download=None):
    ...
    date_column, keys=TABLES[table];cursor='';seen={}
    ...
    while True:
        manifests=ops("""SELECT a.artifact_id,a.domain,a.schema_version,a.checksum,a.row_count,a.metadata_json,
          (SELECT MAX(i.completed_at) FROM data_retention_run_items i WHERE i.status='success'
           ...
           THEN json_extract(i.evidence_json,'$.artifact_id') END=a.artifact_id) release_verified_at
          ...
          AND a.payload_deleted_at IS NULL AND a.artifact_id>? ORDER BY a.artifact_id LIMIT 100""",[domain,cursor])
        if not manifests:return
        for manifest in manifests:
            cursor=manifest['artifact_id'];meta=json.loads(manifest.get('metadata_json') or '{}')
            if meta.get('coverage_start','')>end_date or meta.get('coverage_end','9999')<start_date:continue
            ...
            with sqlite3.connect(':memory:') as db:
                db.row_factory=sqlite3.Row;db.execute(payload['source_schema_sql'])
                for r in missing:
                    ...
                    if key in seen:
                        if seen[key]!=h:raise RuntimeError('retention_market_archive_revision_conflict')
                        continue
                    seen[key]=h;columns=list(clean)
                    db.execute(f'INSERT INTO "{table}" ('+','.join('"'+c+'"' for c in columns)+') VALUES ('+','.join('?' for _ in columns)+')',list(clean.values()))
