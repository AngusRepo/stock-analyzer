...
assert.match(research, /child_receipt_owner=optuna-per-regime/)
const adminControl = fs.readFileSync('src/routes/adminControlRoutes.ts', 'utf8')
assert.match(adminControl, /closeOptunaRunD1Lock\(databaseForDataDomain\(c\.env, 'ops'\)/)
assert.doesNotMatch(adminControl, /closeOptunaRunD1Lock\(c\.env\.DB/)
