...
test('pipeline maturity API preserves canonical lineage and explicit evidence fields', () => {
  ...
  assert.match(source, /scope: 'monitoring'/)
  assert.match(source, /scope: 'diagnostic'/)
  ...
  assert.match(source, /scope: 'prospective_forward'/)
  assert.match(source, /live_evidence_json/)
  assert.match(source, /Locked candidate mature OOS dates/)
  assert.match(source, /prospectiveCandidateStatus/)
  assert.match(source, /maturityProgress\(prospectiveDateCount, EXPECTED_RETURN_PROSPECTIVE_MIN_DATES, 'dates'\)/)
  assert.doesNotMatch(source, /candidateStatus\(l4Serving/)
  assert.doesNotMatch(source, /candidateStatus\(fusionServing/)
  assert.match(source, /Rolling cohort diagnostic corr LCB90/)
  assert.doesNotMatch(source, /gateMetric\('shadow_corr_lcb90'/)
  assert.match(source, /progress: maturityProgress\(dailyEligibleDateCount/)
  assert.match(source, /Latest weekly calibration mature dates/)
  ...
  assert.match(evidenceAdapter, /ALLOCATOR_EV_FUSION_CONTRACT\.validationSchemaVersion/)
  assert.match(evidenceAdapter, /L4_ALPHA_EV_CONTRACT\.modelVersionPrefix/)
  assert.match(expectedReturnManifest, /allocator-ev-fusion-validation-packet-v14/)
  assert.match(expectedReturnManifest, /l4-alpha-ev-ridge-v5-sector-/)
