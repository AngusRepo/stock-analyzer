...
assert(lifecycle.includes("json_extract(r.score_components, '$.version') = 'score_v2'"), 'later filtered recommendation projections must not redefine the canonical learning cohort')
assert(lifecycle.includes('p.stock_id = COALESCE(dr.stock_id, st.id)'), 'snapshot closure must resolve filtered placeholder identities the same way as the producer')
...
assert(lifecycle.match(/allowPointInTimeReconstruction: true/g)?.length === 2, 'allocator lifecycle watchdog must accept the explicit backfill result for operational closure')
