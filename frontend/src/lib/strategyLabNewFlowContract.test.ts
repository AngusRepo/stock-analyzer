...
assert(page.includes('latestBatchInvalid'), 'Strategy cards must distinguish a corrupt decision batch from real per-strategy PIT unavailability')
assert(page.includes('不歸因為各策略 PIT 缺欄'), 'A batch parity failure must not be presented as identical PIT missing fields on every strategy')
assert(page.includes('最新有效 decision date'), 'Pending current date must disclose the latest valid decision date')
...
assert(page.includes('CandidateAtomicV7Dialog') && page.includes('<Dialog open={open}') && page.includes('原始替換評估'), 'Original replacement evidence must open in a modal')
...
assert(page.includes('<StrategyStageTransitionCard') && page.includes("aria-label={isNavOwner ? '原始配對 NAV 替換指標'") && page.includes('查看原始 NAV') && page.includes('if (selectedRow) setAtomicV7StrategyKey'), 'The lower indicator opens candidate or incumbent original evidence')
assert(page.includes('enabled: open && row != null && date != null') && page.includes("['strategy-nav-evidence', row?.id, row?.version, date]"), 'Read original evidence only for the open selected identity/date')
assert(page.includes("sm:max-w-[96vw] xl:max-w-[1500px]") && page.includes("grid-rows-[auto_minmax(0,1fr)]") && page.includes("phase === 'prefilter'") && page.includes("phase === 'pair'") && page.includes("phase === 'cutover'"), 'Wide modal keeps original NAV and collapsible legacy phases in one scroll container')
...
const activationHelpers = page.slice(page.indexOf('function activationGateStatusLabel'), page.indexOf('function atomicV7InapplicabilityLabel'))
assert(!activationHelpers.includes("status === 'pending'") && activationHelpers.includes("status === 'prefilter_failed'") && activationHelpers.includes("status === 'not_evaluated'"), 'Activation helpers must use explicit evidence/prefilter/evaluation states and reject generic pending')
