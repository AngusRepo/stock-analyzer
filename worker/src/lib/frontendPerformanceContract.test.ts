...
const paperRecommendationSource = fs.readFileSync(path.join(frontend, 'components', 'StrategyAbRecommendations.tsx'), 'utf8')
...
assert(paperSource.includes('<StrategyAbRecommendations date={date}') && paperRecommendationSource.includes('recommendationDailyKey(date)'), 'paper room must share the same date-scoped recommendation cache')
