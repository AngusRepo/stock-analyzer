...
def test_l4_training_query_uses_outcome_knowledge_cutoff_after_signal_end_date():
    ...
    assert "'ensemble_v2', json_object(" in captured["sql"]
    assert "'artifact_versions', json(COALESCE(" in captured["sql"]
    assert "'pit_sector_alpha_expert', json(COALESCE(" in captured["sql"]
    assert "\n            p.forecast_data,\n" not in captured["sql"]
    assert "\n            dr.score_components,\n" not in captured["sql"]
    assert "\n            dr.alpha_context,\n" not in captured["sql"]
