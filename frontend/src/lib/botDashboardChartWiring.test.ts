...
assert(page.includes('const activeBuys = isStalePending ? [] : buys'), 'BotDashboard should not treat stale fallback pending buys as today active buys')
assert(page.includes('if (!activeBuys.length)'), 'BotDashboard should fall back to daily recommendations when pending buys are stale or empty')
...
assert(page.includes("(pendingData as any)?.is_stale ? [] : paperPendingBuysFromPayload(pendingData)"), 'BotDashboard performance chart should hide stale pending-buy markers')
