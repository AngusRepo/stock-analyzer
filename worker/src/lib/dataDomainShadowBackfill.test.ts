...
const source = fs.readFileSync('src/lib/dataDomainShadowBackfill.ts', 'utf8')
assert(source.includes("parityScope = 'verified_keyset_batch_manifest'"))
assert(source.includes('coveredSourceRows === sourceRows'))
assert(source.includes('sourceFullChecksum !== targetFullChecksum'))