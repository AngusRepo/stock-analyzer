...
assert(workflows.includes('dispatch_full_fit: true'), 'daily must close input-deduplicated formal releases when maturity is ready')
...
assert(workflows.includes('scheduler_run_id: options.schedulerRunId'), 'Worker must propagate the immutable scheduler run identity into the controller lifecycle')
assert(workflows.includes("String(data.reason ?? '') === 'materialization_job_active'"), 'singleton Cloud Run collisions must create a bounded continuation instead of stranding the scheduler ticket')
...
assert(adminTasks.includes("runActive8OofLifecycle(c.env, requestedRunDate(), 'weekly', schedulerContext)"), 'weekly Active8 scheduler identity must enter the lifecycle call path')
assert(adminTasks.includes("runActive8OofLifecycle(c.env, requestedRunDate(), 'monthly', schedulerContext)"), 'monthly Active8 scheduler identity must enter the lifecycle call path')
...
assert(updateOrchestrator.includes("authority: 'durable_queue'"), 'synchronous continuation closure must settle through the durable queue authority')
...
assert(oofJob.includes('payload["scheduler_ticket_id"] = scheduler_ticket_id'), 'durable job callback must return the exact scheduler ticket')
assert(oofJob.includes('payload["scheduler_run_id"] = scheduler_run_id'), 'durable job callback must return the immutable scheduler run')
