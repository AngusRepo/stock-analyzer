...

{
  ...
  assert(otherRoute.includes("'layer2_coarse_ml_gate'"), 'recommendation API must read formal Layer 2 funnel evidence')
  ...
  assert(screenerFunnelEvidence.includes("'TabM', 'GNN', 'DLinear', 'PatchTST', 'iTransformer', 'TimesFM'"), 'L3 6ML model ids must stay visible in the funnel summary contract')
  ...
  assert(screenerFunnelEvidence.includes('strategy_router_vs_9ml_formal_family_evidence_calibration'), 'L3.5 evidence fusion must expose strategy-vs-9ML calibration method')
...

{
  ...
  assert(marketScreener.includes("downstream_stage: 'layer2_coarse_ml_gate'"), 'Worker ML slate queue must point to ml-controller formal L2 owner without owning L2')
