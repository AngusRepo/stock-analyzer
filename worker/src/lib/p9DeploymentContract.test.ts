...
assert(workflow.includes('--sbom=true --provenance=mode=max'), 'P9 workflow must build image SBOM and provenance attestations')
assert(workflow.includes('--file ml-service/Dockerfile --tag stockvision-ml-service:'), 'P9 workflow must build the ML service image independently')
assert(workflow.includes('stockvision-ml-service:${{ github.sha }}'), 'P9 workflow must scan the ML service image')
