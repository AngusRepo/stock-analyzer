...
assert(workbench.includes("label: 'LIVE IC'") && workbench.includes('daily rolling live IC'), 'LIVE IC cells should show daily rolling verified IC instead of old shadow/parity gate language')
...
assert(workbench.includes('PBO policy missing'), 'PBO/CPCV cell should surface missing adaptive policy instead of falling back to a fixed threshold')
...
assert(page.includes('Promotion reason：') && page.includes('gate evidence:'), 'Promotion audit should explain why the controller decided to promote or block')
assert(page.includes('Rollback target') && page.includes('current_champion_version'), 'Promotion audit should expose the old champion as rollback target')
assert(page.includes('Decision source') && page.includes('AUTO / promotion controller'), 'Auto promotion should expose its controller decision source')
assert(page.includes('Decision time') && page.includes('API not exposed'), 'Promotion audit must disclose when decided_at is not available instead of inventing local freshness')
assert(!page.includes('Auto promote pointer'), 'Auto promotion should not require a misleading manual UI button')
assert(page.includes('UI audit only'), 'Auto promotion rows should explain that controller execution is automatic')
assert(workbench.includes("url.searchParams.set('model', id)"), 'Evidence matrix selection should be URL-keyed and shareable')
