...
Tree and sequence models run in separate Modal jobs. This module keeps the join
contract explicit so the orchestrator can fail closed and train the rank stacker
from standard OOS artifacts instead of relying on one monolithic job.
"""
...
def _ordered_model_names(names: Iterable[str]) -> list[str]:
    order = {name: idx for idx, name in enumerate(TREE_MODELS)}
