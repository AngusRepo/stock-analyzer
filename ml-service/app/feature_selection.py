...
def _train_lgbm_regression(X_train: np.ndarray, y_train: np.ndarray,
    ...
    try:
        _test_lgb = _lgb.LGBMRegressor(device="gpu", n_estimators=1, verbose=-1)
        _test_lgb.fit(np.zeros((10, 2)), np.zeros(10))
        params["device"] = "gpu"
        params["gpu_use_dp"] = False
        print("[TargetPerm] LightGBM GPU mode detected")
    except Exception:
...
def run_feature_selection_pipeline(
    ...
    **_kwargs,  # absorb legacy params (required_power etc.)
    ...
    embargo_days = 15  # match triple-barrier max_days=20
