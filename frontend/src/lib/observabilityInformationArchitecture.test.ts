...
assert(page.includes("event.domain === 'ml_threshold_policy'"), 'OBS adaptive/meta panel must consume ml_threshold_policy runtime events')
assert(page.includes('Runtime ML threshold policy'), 'OBS should display runtime ML threshold policy provenance beside adaptive delta')
