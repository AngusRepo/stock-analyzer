...
assert.match(source, /LEARNING_HOT_RETENTION_DAYS = 120/)
...
assert.match(adminReadSource, /ROW_NUMBER\(\) OVER \(\s*PARTITION BY r\.policy_id/)
assert.match(adminReadSource, /WHERE executor_policy\.action='archive_scrub'/)
assert.match(adminReadSource, /r\.ordinal=1/)
assert.match(adminReadSource, /r\.completed_at >= datetime\('now', '-7 days'\)/)
assert.match(adminReadSource, /p\.action='archive_scrub'/)
assert.match(adminReadSource, /r\.run_id LIKE 'retention-hot-window-drain:%'/)
assert.doesNotMatch(adminReadSource, /MAX\(CASE WHEN r\.status='success' THEN 1 ELSE 0 END\) AS operational/)

assert.match(adminReadSource, /legacy-hot-data-retirement:%/)
assert.match(adminReadSource, /oof-hot-data-retirement:%/)
assert.match(handlers, /beginRetentionRun/)
assert.match(handlers, /legacy_hot_r2_v1/)
