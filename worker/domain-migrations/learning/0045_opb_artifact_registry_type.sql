-- Admit OPB arm-prior artifacts without disabling the registry CHECK constraint.
-- Preserve every registry/payload row, index and immutable identity; no promotion.
-- Revision triggers must be reinstalled by the existing protected binding task.
...
DROP INDEX IF EXISTS idx_model_artifact_registry_identity_v3;
DROP INDEX IF EXISTS idx_expected_return_artifact_payloads_owner;
DROP INDEX IF EXISTS idx_expected_return_artifact_payloads_version;

ALTER TABLE expected_return_artifact_payloads
  RENAME TO expected_return_artifact_payloads_opb_type_v4_legacy;
ALTER TABLE model_artifact_registry
  RENAME TO model_artifact_registry_opb_type_v4_legacy;
...
CREATE TABLE model_artifact_registry (
  ...
  candidate_type TEXT NOT NULL CHECK(candidate_type IN (
    ...
    'allocator_ev_fusion_refresh','opb_arm_prior_refresh','unknown'
  )),
...
SELECT
  ...
  live_gate_status, live_evidence_json, promotion_decision, approval_state,
  created_at, updated_at
FROM model_artifact_registry_opb_type_v4_legacy;
...
SELECT
  ...
  payload_checksum, source_artifact_path, source_artifact_checksum,
  source_cohort_id, created_at, updated_at
FROM expected_return_artifact_payloads_opb_type_v4_legacy;
...
DROP TABLE expected_return_artifact_payloads_opb_type_v4_legacy;
DROP TABLE model_artifact_registry_opb_type_v4_legacy;
