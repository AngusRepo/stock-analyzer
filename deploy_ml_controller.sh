...
FINLAB_EXECUTION_LOOP_ENABLED="${FINLAB_EXECUTION_LOOP_ENABLED:-true}"
...
build_verify_job_env_file() {
  ...
  FINLAB_EXECUTION_LOOP_ENABLED="$FINLAB_EXECUTION_LOOP_ENABLED" \
...
envs["FINLAB_BACKFILL_EXECUTOR"] = "modal"
envs["FINLAB_EXECUTION_LOOP_ENABLED"] = os.environ.get("FINLAB_EXECUTION_LOOP_ENABLED", "true")
...
if ! gcloud run deploy "$SERVICE" \
    ...
    --region="$REGION" \
    --timeout=3600 \
    --update-env-vars="ENVIRONMENT=production,GCS_BUCKET_NAME=${GCS_BUCKET_NAME},RETRAIN_LOCK_BUCKET=${RETRAIN_LOCK_BUCKET},GCP_PROJECT_ID=${GCP_PROJECT_ID},GCP_REGION=${GCP_REGION},PIPELINE_JOB_NAME=${PIPELINE_JOB_NAME},VERIFY_JOB_NAME=${VERIFY_JOB_NAME},OPTUNA_JOB_NAME=${OPTUNA_JOB_NAME},FINLAB_JOB_NAME=${FINLAB_JOB_NAME},FINLAB_BACKFILL_EXECUTOR=modal,STOCKVISION_WORKER_URL=${STOCKVISION_WORKER_URL},FINLAB_EXECUTION_LOOP_ENABLED=${FINLAB_EXECUTION_LOOP_ENABLED}" \
    --update-secrets="$RUN_SECRET_BINDINGS" \
    --quiet; then
...
if ! gcloud run jobs update "$JOB" \
    ...
    --image="$NEW_IMAGE" \
    --update-secrets="$RUN_SECRET_BINDINGS" \
    --update-env-vars="ENVIRONMENT=production,GCS_BUCKET_NAME=${GCS_BUCKET_NAME},RETRAIN_LOCK_BUCKET=${RETRAIN_LOCK_BUCKET},GCP_PROJECT_ID=${GCP_PROJECT_ID},GCP_REGION=${GCP_REGION},PIPELINE_JOB_NAME=${PIPELINE_JOB_NAME},VERIFY_JOB_NAME=${VERIFY_JOB_NAME},OPTUNA_JOB_NAME=${OPTUNA_JOB_NAME},FINLAB_JOB_NAME=${FINLAB_JOB_NAME},FINLAB_BACKFILL_EXECUTOR=modal,STOCKVISION_WORKER_URL=${STOCKVISION_WORKER_URL},FINLAB_EXECUTION_LOOP_ENABLED=${FINLAB_EXECUTION_LOOP_ENABLED}"; then
