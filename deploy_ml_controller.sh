...
ML_CONTROLLER_SERVICE_ACCOUNT="${ML_CONTROLLER_SERVICE_ACCOUNT:-stockvision-ml-controller@${GCP_PROJECT_ID}.iam.gserviceaccount.com}"
PIPELINE_JOB_SERVICE_ACCOUNT="${PIPELINE_JOB_SERVICE_ACCOUNT:-stockvision-pipeline@${GCP_PROJECT_ID}.iam.gserviceaccount.com}"
ML_CONTROLLER_CONCURRENCY="${ML_CONTROLLER_CONCURRENCY:-8}"
ML_CONTROLLER_MIN_INSTANCES="${ML_CONTROLLER_MIN_INSTANCES:-1}"
ML_CONTROLLER_MAX_INSTANCES="${ML_CONTROLLER_MAX_INSTANCES:-25}"
...
ML_SERVICE_SECRET_SECRET="${ML_SERVICE_SECRET_SECRET:-stockvision-ml-service-secret:latest}"
...
BASE_SECRET_BINDINGS="FINLAB_API_KEY=${FINLAB_API_KEY_SECRET},CF_API_TOKEN=${CF_API_TOKEN_SECRET},STOCKVISION_AUTH_TOKEN=${STOCKVISION_AUTH_TOKEN_SECRET},ML_CONTROLLER_SECRET=${ML_CONTROLLER_SECRET_SECRET},ML_SERVICE_SECRET=${ML_SERVICE_SECRET_SECRET},MODAL_TOKEN_ID=${MODAL_TOKEN_ID_SECRET},MODAL_TOKEN_SECRET=${MODAL_TOKEN_SECRET_SECRET}"
...
REQUIRED_ENV_VARS=(
  ...
  ML_CONTROLLER_SERVICE_ACCOUNT
  PIPELINE_JOB_SERVICE_ACCOUNT
  ML_CONTROLLER_CONCURRENCY
  ML_CONTROLLER_MIN_INSTANCES
  ML_CONTROLLER_MAX_INSTANCES
...
require_pinned_secret_ref() {
  local var_name="$1"
  local value="${!var_name:-}"
  require_nonempty "$var_name" "Use a Secret Manager reference pinned to a numeric version, e.g. secret-name:7"
  if [[ ! "$value" =~ :[1-9][0-9]*$ ]]; then
    echo "ERROR: $var_name must pin a numeric Secret Manager version; aliases such as :latest are forbidden for production releases." >&2
    exit 7
  fi
}
...
run_preflight() {
  ...
  require_nonempty "ML_SERVICE_SECRET_SECRET" "Secret Manager reference for ML service auth token, e.g. stockvision-ml-service-secret:latest"
  ...
  for secret_ref_var in \
    CF_API_TOKEN_SECRET \
    STOCKVISION_AUTH_TOKEN_SECRET \
    FINLAB_API_KEY_SECRET \
    ML_CONTROLLER_SECRET_SECRET \
    ML_SERVICE_SECRET_SECRET \
    MODAL_TOKEN_ID_SECRET \
    MODAL_TOKEN_SECRET_SECRET \
    SHIOAJI_API_KEY_SECRET \
    SHIOAJI_SECRET_KEY_SECRET \
    SHIOAJI_ACCOUNT_ID_SECRET \
    SHIOAJI_CERT_PERSON_ID_SECRET \
    SHIOAJI_CERT_PASSWORD_SECRET \
    SHIOAJI_CERT_PFX_SECRET; do
    require_pinned_secret_ref "$secret_ref_var"
  done

  for service_account in "$ML_CONTROLLER_SERVICE_ACCOUNT" "$PIPELINE_JOB_SERVICE_ACCOUNT"; do
    if ! gcloud iam service-accounts describe "$service_account" --project="$GCP_PROJECT_ID" >/dev/null 2>&1; then
      echo "ERROR: required dedicated service account does not exist: $service_account" >&2
      exit 7
    fi
  done
  ...
  print_preflight_value "ML_SERVICE_SECRET_SECRET"
...
if ! gcloud run deploy "$SERVICE" \
    ...
    --region="$REGION" \
    --timeout=3600 \
    --concurrency="$ML_CONTROLLER_CONCURRENCY" \
    --min-instances="$ML_CONTROLLER_MIN_INSTANCES" \
    --max-instances="$ML_CONTROLLER_MAX_INSTANCES" \
    --cpu-boost \
    --service-account="$ML_CONTROLLER_SERVICE_ACCOUNT" \
...
if ! gcloud run jobs update "$JOB" \
    --region="$REGION" \
    --image="$NEW_IMAGE" \
    --service-account="$PIPELINE_JOB_SERVICE_ACCOUNT" \
    --update-secrets="$RUN_SECRET_BINDINGS" \
...
if [ "$WITH_MODAL" = "1" ]; then
  ...
  if [ -z "${ADMIN_API_TOKEN:-}" ] || [ -z "${ADMIN_MUTATION_APPROVAL_ID:-}" ]; then
    echo "❌ ERROR: ADMIN_API_TOKEN and ADMIN_MUTATION_APPROVAL_ID are required for --with-modal" >&2
    exit 6
  fi
  CTOKEN="${ML_CONTROLLER_TOKEN}"
  ATOKEN="${ADMIN_API_TOKEN}"
  APPROVAL_ID="${ADMIN_MUTATION_APPROVAL_ID}"
  ...
  HTTP_STATUS=$(curl -sS -o "$MODAL_RESP_FILE" -w "%{http_code}" \
      ...
      -H "X-Admin-Token: $ATOKEN" \
      -H "X-Admin-Approval-Id: $APPROVAL_ID" \
