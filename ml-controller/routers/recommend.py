...
from services.scorer import score_and_rank, StockScore

logger = logging.getLogger(__name__)
router = APIRouter()

# Fallback: Controller 自己也能讀（若 Worker 不傳）
...
class RecommendRequest(BaseModel):
    ...
    anthropic_api_key: Optional[str] = None   # Worker 明文傳入（Worker 有 env var）
    top_n: int = 5
...
def post_recommend(req: RecommendRequest):
    ...
    reasons = _llm_reasons(api_key, top, req.sectors) if api_key else []
