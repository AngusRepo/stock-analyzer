...
const activeStrategyCounts = activeStrategiesRaw.map((strategy) => {
  ...
  return {
    ...
    status_today: 'active',
    runtime_pool_status: poolStatus.status ?? '',
    daily_match_status: poolStatus.daily_match_status ?? (strict ? 'strict_match' : 'strict_empty_threshold'),
    missing_evidence: Array.isArray(poolStatus.missing_evidence) ? poolStatus.missing_evidence.join('|') : '',
    strict_match_count: strict?.symbols ?? 0,
    near_match_count: poolStatus.near_match_count ?? 0,
