...
assert(workbench.includes('Evidence matrix'), 'Model Pool cockpit must show weekly IC, OOS, live, PBO/CPCV, final compare, and state in one matrix')
assert(workbench.includes('Gate & incidents'), 'Model Pool cockpit must merge alert queue and gate drilldown into one focused panel')
assert(workbench.includes('Incidents queue'), 'Merged gate panel must keep blocker and promotion incidents visible')
...
assert(workbench.includes('OOS IC') && workbench.includes('PBO/CPCV') && workbench.includes('FINAL'), 'Evidence matrix must include OOS IC, PBO/CPCV, and final compare gates')
assert(workbench.includes('Missing evidence'), 'Evidence table must expose missing evidence chips')
assert(page.includes('modelUpgradeStatusReady={modelUpgradeStatus.isSuccess}'), 'Model Pool page must pass model-upgrade status readiness into the cockpit')
...
assert(workbench.includes('dataset, pointer, promotion pressure, and missing evidence'), 'Model Pool cockpit must show dataset, pointer, promotion pressure, and missing evidence in the evidence table')
