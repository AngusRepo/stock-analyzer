...
assert(source.includes('Object.entries(options.canonicalRunIds ?? {})'), 'calibration loader must iterate the cross-D1 canonical owner map')
assert(source.includes('r.signal_date=? AND r.producer_run_id=?'), 'calibration loader must query each canonical reference owner exactly')
assert(source.includes('e.signal_date=? AND e.producer_run_id=?'), 'calibration loader must query each canonical backfill owner exactly')
assert(!source.includes('h.key=c.signal_date'), 'calibration loader must not rescan the full lookback through per-row json_each checks')
