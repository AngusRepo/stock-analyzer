...
const JOB_DEFS: JobDef[] = [
  ...
  { id: 'pipeline', name: 'Pipeline', schedule: 'After screener + regime', cron: '', group: 'pipeline_chain', chainIndex: 8 },
  { id: 'ml-predict', name: 'ML Predict', schedule: 'Inside pipeline', cron: '', group: 'pipeline_chain', chainIndex: 9 },
  { id: 'recommendation', name: 'Daily Recommendation', schedule: 'Inside pipeline', cron: '', group: 'pipeline_chain', chainIndex: 10 },
  { id: 'post-pipeline-chain', name: 'Post Pipeline Callback', schedule: 'After pipeline callback', cron: '', group: 'pipeline_chain', chainIndex: 11 },
  { id: 'verify-v2', name: 'Verify (V2 LangGraph)', schedule: 'After pipeline callback', cron: '', group: 'pipeline_chain', chainIndex: 12 },
  { id: 'post-verify-chain', name: 'Post Verify Callback', schedule: 'After verify callback', cron: '', group: 'pipeline_chain', chainIndex: 13 },
  { id: 'model-ic-tracker', name: 'Model IC Tracker', schedule: 'After verify callback / Friday full check', cron: '30 11 * * 5', group: 'pipeline_chain', chainIndex: 14 },
  { id: 'linucb-reward-ledger', name: 'LinUCB Reward Ledger', schedule: 'After rolling IC', cron: '', group: 'pipeline_chain', chainIndex: 15 },
  { id: 'adapt', name: 'Adapt Params', schedule: 'After LinUCB ledger', cron: '', group: 'pipeline_chain', chainIndex: 16 },
  { id: 'daily-report', name: 'Daily Report', schedule: 'After adapt', cron: '', group: 'pipeline_chain', chainIndex: 17 },
  { id: 'paper-active-postmarket', name: 'Paper Active Postmarket', schedule: 'After daily report', cron: '', group: 'pipeline_chain', chainIndex: 18 },
  { id: 'obsidian-sync', name: 'Obsidian Sync', schedule: 'After paper-active postmarket', cron: '', group: 'pipeline_chain', chainIndex: 19 },
  { id: 'meta-learning-shadow', name: 'Meta Learning Shadow', schedule: 'After obsidian sync', cron: '', group: 'pipeline_chain', chainIndex: 20 },
  { id: 'strategy-learning', name: 'Strategy Learning', schedule: 'After meta shadow / historical reruns', cron: '', group: 'pipeline_chain', chainIndex: 21 },
