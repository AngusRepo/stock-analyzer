...
import { alignStrategyAb, alignPaperPrimary } from '@/lib/strategyAbLive'
...
export default function StrategyAbLive({ data, date }: { data?: PairedNavShadowReadModel; date: string }) {
  ...
  const primaryPair = bs.length === 1 && !!bs[0].comparison?.strategy_ab?.baseline_primary
  const comparison = primaryPair ? (b.data ? alignPaperPrimary(b.data) : null) : a.data && b.data ? alignStrategyAb(a.data, b.data) : null
  const rows = [['A 主方案', primaryPair ? b.data : a.data, primaryPair ? 'baseline' : 'candidate'], ['B 挑戰方案', b.data, 'candidate']] as const
  return <section className={panel} aria-label="A B 每日實際比較">
    ...
    <p className="mt-2 text-xs leading-5 text-slate-400">A 為指定 Paper 主方案，以明確試驗資格啟用；NAV 持續審查績效。B 與 A 使用同起點的配對模擬帳本比較，不自動切換策略。</p>
    ...
    {!data || data.status === 'unavailable' ? <p role="status" className="mt-4 text-amber-200">每日來源尚未取得或驗證失敗。</p>
      : (!primaryPair && as.length !== 1) || bs.length !== 1 ? <p role="status" className="mt-4 text-slate-400">尚未形成同一實驗的 A/B 完整帳本。下方歷史研究不會填入每日績效。</p>
        : (!primaryPair && a.isError) || b.isError ? <p role="alert" className="mt-4 text-amber-200">A/B 帳本讀取失敗，請重新整理。</p>
