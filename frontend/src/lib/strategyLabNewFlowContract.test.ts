...
assert(page.includes('AtomicReplacementSummary'), 'Cross-family replacement must remain available')
const governanceDetailsStart = page.indexOf('治理與權責詳情：Production firewall')
const governanceDetailsEnd = page.indexOf('</details>', governanceDetailsStart)
assert(page.indexOf('<AtomicReplacementSummary', governanceDetailsEnd) > governanceDetailsEnd, 'Atomic V7 complete gates must render outside the collapsed governance details')
assert(page.includes('Atomic V7 完整門檻總表（固定顯示）') && page.includes('A. Candidate prefilter（先決條件）') && page.includes('B. Candidate vs Active 同日 pair') && page.includes('C. 完整 portfolio 與原子 cutover'), 'Atomic V7 must keep the complete three-level gate list visible without opening disclosures')
assert(page.includes('prefilter-failed = 已有 evidence') && page.includes('evidence-pending = 尚無可判定 evidence') && page.includes('prefilterFailedCount') && page.includes('evidencePendingCount'), 'Atomic V7 must explain and count failed prefilters separately from missing evidence')
