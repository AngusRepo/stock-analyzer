...

{
  const summary = formatPendingBuyCronSummary('morning setup done', {
    ...
    active_count: 3,
    total_count: 3,
    execution_counts: { pending: 3, filled: 0, skipped: 0, cancelled: 0, expired: 0 },
    debate_counts: { pending: 3, completed: 0, failed: 0, skipped: 0 },
  })

  assert(summary === 'morning setup done; state=debate_pending(等待辯論); active=3/3; debate_pending=3; exec[pending=3 filled=0 skipped=0 cancelled=0 expired=0]', 'morning summary should include canonical pending-buy state')
}

{
  const summary = formatPendingBuyCronSummary('intraday heartbeat ok', {
    ...
    active_count: 0,
    total_count: 4,
    execution_counts: { pending: 0, filled: 2, skipped: 1, cancelled: 1, expired: 0 },
  ...
  assert(summary.includes('exec[pending=0 filled=2 skipped=1 cancelled=1 expired=0]'), 'closed summary should include execution counts')
