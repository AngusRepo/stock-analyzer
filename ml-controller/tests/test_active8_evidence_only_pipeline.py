...
def test_writer_persists_optional_sequence_missingness_without_fake_score(monkeypatch, formal) -> None:
    statements: list[tuple[str, list]] = []
    monkeypatch.setattr(
        recommendation_service,
        "_predictions_batch_execute",
    ...
    written = recommendation_service.write_predictions_to_d1(
        {
            "2330": {
                ...
                "active8_action_authority": None if formal else _authority(),
                "ensemble_v2": {"signal": "HOLD"} if formal else None,
            }
        },
        {"2330": 1},
        "2026-08-26",
    ...
    all_inserts = [(sql, params) for sql, params in statements if sql.lstrip().startswith("INSERT INTO predictions")]
    assert written == len(all_inserts)
    if formal:
        assert len(all_inserts) == 1 + len(core_models) + len(recommendation_service.ACTIVE_ALPHA_MODELS)
    inserts = [(sql, params) for sql, params in all_inserts if isinstance(params[1], str) and params[1].endswith("::challenger")]
    assert len(inserts) == len(recommendation_service.ACTIVE_ALPHA_MODELS)
    assert {params[1] for _sql, params in inserts} == {
