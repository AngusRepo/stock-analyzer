...
## 2026-09-06 release readback follow-up

- Original 8/20 chip, institutional, futures and regime-context inputs restored with per-field readback before Ops acknowledgment; historical risk is 28/yellow using only as-of inputs. Foreign subtotal identity no longer collides with foreign-ex-dealer; historical dealer amounts since 5/15 were verified zero, so no historical numeric rewrite was needed.
- Native daily collector now captures a compact full-source trading calendar before clipping prices to the daily date. Calendar facts are written before prices; Ops acknowledgment remains last. The actual collector call-path regression and 126 scoped Python tests pass.
- First native label reprojection completed 5 canonical date groups and 17 multi-horizon groups without deferred work. Readback found two retained 10-day label identities absent from hot candidates; repaired the native identity/date union rather than deleting or ignoring those labels. Worker retained-identity, horizon/retry/terminal tests and full TypeScript checks pass.
- Pending release operations: commit/fast-forward the follow-up, deploy the final source to all affected runtimes, reproject the two retained-label dates and verify all before/after values; refresh derived outcomes/read models; verify unchanged formal pointers and honest IPO shadow maturity. Source snapshots, requests and readback receipts remain in the release outbox, not in the deploy bundle.
