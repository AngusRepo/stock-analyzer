...
def _rank_to_l2_signal(rank_score: float) -> str:
    if rank_score >= 0.82:
        return "STRONG_BUY"
    if rank_score >= 0.60:
        return "BUY"
...
def _last_close_from_payload(payload: dict) -> float:
...
def predict_l2_tree_batch(payloads: list[dict]) -> dict:
    """Run the cheap L2 coarse gate models only.

    This endpoint intentionally excludes TabM, GNN, sequence models, and
    state-space overlays. L3 owns those heavier families after L2 has reduced
    the candidate set.
    """
    request_cls, _predict_fn = _runtime()
    payloads = payloads or []
    contexts_by_position: dict[int, _FeatureBatchContext] = {}
    results: list[dict | None] = [None] * len(payloads)

    for idx, payload in enumerate(payloads):
        try:
            req = request_cls(**payload)
            contexts_by_position[idx] = _build_feature_batch_context(req)
        except Exception as exc:  # noqa: BLE001 - one bad symbol must not kill the batch.
            results[idx] = _error_result(payload, exc)

    contexts = list(contexts_by_position.values())
    if contexts:
    ...
    for idx, ctx in contexts_by_position.items():
        payload = payloads[idx]
        scores = {
            name: float(score)
            for name, score in ctx.rank_scores.items()
            if name in _L2_TREE_MODEL_NAMES
        }
        if not scores:
            results[idx] = {
                **_error_result(payload, Exception("; ".join(ctx.model_errors or ["L2 tree models emitted no score"]))),
                "source": "l2_tree_predict",
                "prediction_stage": "L2",
            }
            continue

        avg_rank = float(np.mean(list(scores.values())))
        signal = _rank_to_l2_signal(avg_rank)
        confidence = min(0.90, 0.50 + abs(avg_rank - 0.50) * 0.80 + len(scores) * 0.04)
        forecast_pct = (avg_rank - 0.50) * 0.12
        current_price = _last_close_from_payload(payload)
        results[idx] = {
            "stock_id": getattr(ctx.req, "stock_id", payload.get("stock_id", 0)),
            "symbol": getattr(ctx.req, "symbol", payload.get("symbol", "?")),
            "current_price": current_price,
            "signal": signal,
            "direction": _direction_from_signal(signal),
            "confidence": round(float(confidence), 4),
            "consensus": len(scores),
            "forecast_pct": round(float(forecast_pct), 6),
            "forecast_range": [round(float(forecast_pct), 6), round(float(forecast_pct), 6)],
            "signal_strength": round(abs(float(forecast_pct)), 6),
            "reasoning": "L2 cheap tree coarse gate",
            "entry_price": round(current_price, 2) if current_price > 0 else None,
            "stop_loss": round(current_price * 0.96, 2) if current_price > 0 else None,
            "target1": round(current_price * 1.03, 2) if current_price > 0 else None,
            "target2": round(current_price * 1.06, 2) if current_price > 0 else None,
            "models": [
                {
                    "name": name,
    ...
    return {
        ...
        "metrics": {
            "runtime": "tree_feature_batch",
            "contract": "l2_tree_predict_v1",
            "models": list(_L2_TREE_MODEL_NAMES),
            "error_summary": error_summary,
        },
    }
