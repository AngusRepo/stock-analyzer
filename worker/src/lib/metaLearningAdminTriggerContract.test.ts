...
const metaController = fs.readFileSync('src/lib/adaptiveMetaPolicyController.ts', 'utf8')
...
assert(taskMap.includes('reconcileAdaptiveMetaPolicy'), 'persisted weekly replay must invoke the guarded dynamic Meta controller')
...
assert(adaptiveEngine.includes('params.model_allocator = current.model_allocator'), 'daily adaptive refresh must preserve the Meta-owned serving policy')
assert(
  metaController.includes('const CANARY_STREAK = 2') &&
    metaController.includes('const ACTIVE_STREAK = 4') &&
    metaController.includes('const CANARY_CAP = 0.05') &&
    metaController.includes('const MAX_CAP = 0.15'),
  'Meta policy must use bounded 2-pass canary and 4-pass activation',
)
assert(metaController.includes("rollback_policy: 'first_failed_or_stale_weekly_replay_returns_to_baseline'"), 'Meta serving policy must declare automatic rollback')
