...
def test_payload_builder_does_not_fallback_to_legacy_financials() -> None:
    source = (ROOT / "services" / "payload_builder.py").read_text(encoding="utf-8")
    ...
    assert 'decision_date=state["run_date"]' in (ROOT / "graphs" / "daily_pipeline_v2.py").read_text(encoding="utf-8")
...
def test_snapshot_and_recommendation_fundamentals_enforce_bitemporal_cutoff() -> None:
    snapshot = (ROOT / "services" / "dataset_snapshot_exporter.py").read_text(encoding="utf-8")
    recommendation = (ROOT / "services" / "recommendation_service.py").read_text(encoding="utf-8")
