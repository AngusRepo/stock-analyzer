...
export const SCHEDULER_DEPENDENCY_MAP: Record<string, SchedulerDependencySpec> = {
  'evening-chain': {
    ...
    currentFunction: 'Post-market root chain: update -> indicators -> screener -> regime -> S12 structure snapshot -> allocator EV readiness -> pipeline, then callback chains.',
    upstream: [],
    downstream: ['indicator-queue', 'screener', 'regime-compute', 's12-structure-snapshot', 'allocator-ev-readiness', 'pipeline', 'post-pipeline-chain', 'post-verify-chain'],
    requiredBeforeDisable: [],
    operatorRisk: 'high',
    recommendation: 'Keep as the single post-market root scheduler.',
  },
  's12-structure-snapshot': {
    task: 's12-structure-snapshot',
    owner: 'worker_chain',
    consolidationClass: 'merge_into_chain',
    currentFunction: 'Materializes the S12 structure snapshot after regime computation and resumes the post-screener callback chain.',
    replacementOwner: 'evening-chain/updateOrchestrator post-screener stage',
    upstream: ['screener', 'regime-compute'],
    downstream: ['allocator-ev-readiness', 'pipeline'],
    requiredBeforeDisable: [
      'S12 snapshot callback must reach a terminal state for the active run date',
      'allocator EV readiness must not start before S12 snapshot completion',
    ],
    operatorRisk: 'high',
    recommendation: 'Keep as a first-class event-driven chain stage with realtime runtime evidence.',
  },
  'allocator-ev-readiness': {
    ...
    requiredBeforeDisable: [
      ...
      'allocator EV fusion refresh failure must clear stale fusion config and continue with L4',
    ],
    operatorRisk: 'high',
