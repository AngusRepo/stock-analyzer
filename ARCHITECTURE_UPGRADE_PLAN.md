...
## 七、Safari LINE 分享問題（附錄）

### 問題

Safari 的 Universal Link 機制要求 `window.location.href` 必須在使用者手勢的**同步呼叫堆疊**中觸發。`await saveProductToDB()` 打斷了手勢鏈，導致 Safari 不會觸發 Universal Link 開啟 LINE App。

### 解法

```javascript
const lineDialogShareHandler = () => {
  // 同步階段先開窗（在手勢鏈內，不被擋）
  const newWindow = window.open('about:blank', '_blank');
  newWindow.document.write('<p>正在載入，請稍候...</p>');

  saveProductToDB("line").then((groupId) => {
    const link_url = import.meta.env.VITE_SHARE_URL + "my-estimation/share/" + groupId;
    const quote = "嗨，我在國泰人壽找到不錯的商品，想跟你分享～\n";
    const quoteUrl = encodeURIComponent(quote + link_url);
    const lineUrl = "https://line.me/R/msg/text/?" + quoteUrl;
...
- `.then` 和 `await` 對 Safari 來說一樣，都會打斷手勢鏈
- `window.open` 必須在 `await` / `.then` 之前呼叫
- App WebView 內 `window.open(url)` 應直接帶完整 URL，不要開空窗再填（WebView 可能只檢查開窗時的 URL 決定是否開外部 App）
- 如果需要確保 DB 存檔完成，可用 `navigator.sendBeacon` 或兩步驟 UI
