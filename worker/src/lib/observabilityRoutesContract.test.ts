...
assert(readRoutes.includes('.filter((point) => point.observed_date < today)'), 'Capacity growth must compare against a prior date, never the current-day ledger row')
assert(readRoutes.includes('buildStorageCapacityGrowthEstimate') && fs.readFileSync('src/lib/storageCapacityTelemetry.ts', 'utf8').includes('awaiting_post_cutover_observations'), 'Legacy growth projection must wait for a post-cutover capacity baseline')
assert(readRoutes.includes('const legacyBaseline = [...baselineByDomain.values()].sort().at(-1) ?? null'), 'Capacity route must anchor the legacy baseline to the latest domain cutover date')
