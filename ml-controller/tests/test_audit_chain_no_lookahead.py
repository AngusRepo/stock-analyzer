...
def test_chain_no_lookahead_audit_treats_query_error_as_failure():
    result = run_audit(lambda sql, params=None, timeout=60.0: [])

    assert result["decision"] == "FAIL"
    assert result["failed_checks"] == [name for name, _, _ in CHECKS]
    assert all(row["status"] == "ERROR" for row in result["checks"])