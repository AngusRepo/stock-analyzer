...
def test_controller_deploy_is_fail_closed_on_identity_and_provenance() -> None:
    ...
    assert script.count('--update-labels="$PROVENANCE_LABELS"') == 11
    assert script.count('--labels="$PROVENANCE_LABELS"') == 9


def test_every_shared_cloud_run_job_receives_release_provenance_labels() -> None:
    script = (ROOT / "deploy_ml_controller.sh").read_text(encoding="utf-8")

    # Service + primary pipeline Job + nine shared Job update paths.
    assert script.count('--update-labels="$PROVENANCE_LABELS"') == 11
    # All nine shared Job create paths carry the same release identity.
    assert script.count('--labels="$PROVENANCE_LABELS"') == 9
