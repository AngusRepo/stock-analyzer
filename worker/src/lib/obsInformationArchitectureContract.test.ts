...
assert(obs.includes('data-testid="obs-signal-board"'), 'OBS must show a visual signal board before dense operational rows')
assert(obs.includes('Scheduler SLO') && obs.includes('Deploy Gate') && obs.includes('OBS Events'), 'OBS signal board must compress reliability and incident state into visual metrics')
assert(obs.includes('sv-content-card') && obs.includes('sv-disclosure'), 'OBS cards and drilldowns must consume operation surface tokens')
...
assert(dataQuality.includes('data-testid="data-quality-signal-board"'), 'Data Quality drilldown should compress trust and gaps before dense checks')
assert(dataQuality.includes('sv-content-card') && dataQuality.includes('sv-accent-text'), 'Data Quality drilldown should consume the shared operations surface')
