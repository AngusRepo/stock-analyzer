...
export const MODEL_UPGRADE_CANDIDATES: ModelUpgradeCandidate[] = [
  {
    ...
    titleZh: '殘差 MLP',
    roleZh: '補 tree / FT 沒捕到的表格殘差信號；可做 shadow predict，但不直接投票。',
  ...
  },
  {
    ...
    titleZh: '跨股票圖模型',
    roleZh: '用產業、概念、供應鏈關係評估 cross-stock graph alpha；目前不進 production vote。',
  ...
  },
  {
    ...
    roleZh: '與 Chronos family 做 foundation model 對照；不直接替換 production。',
...
export const MODEL_UPGRADE_STAGE_LABELS: Record<ModelUpgradeStage, string> = {
  ...
  meta_optimizer: 'meta optimizer / 參數最佳化器',
