...
import { backfillDataDomainTableShadow, type DomainShadowBackfillResult } from './dataDomainShadowBackfill'
...
async function nextIncompleteTable(env: Bindings, domain: DataDomain): Promise<string | null> {
  ...
  const completedSet = new Set((completed.results ?? []).map((row) => String(row.table_name)))
...
export async function processDataDomainShadowBackfillDrain(
  ...
  if (!table) {
    ...
    await logSchedulerResult(env.KV, 'data-domain-shadow-backfill', {
      status: 'success',
      summary: `domain=${domain} initial_copy_complete run_id=${runId}`,
      duration_ms: 0,
      run_id: runId,
