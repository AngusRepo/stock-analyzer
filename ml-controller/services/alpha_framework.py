...
def normalize_alpha_policy(raw: dict | None = None) -> dict[str, Any]:
    ...
    allocation_engine = str(
        _camel_or_snake(raw_alloc, "engine", "engine", alloc_default["engine"])
        or alloc_default["engine"]
    ).strip()
    allocation_controller = str(
        _camel_or_snake(raw_alloc, "controller", "controller", alloc_default["controller"])
        or alloc_default["controller"]
    ).strip()
