...
CHECKS: tuple[tuple[str, str, list[Any]], ...] = (
    ...
    ("canonical_selection_label", """
        SELECT COUNT(*) AS violations FROM canonical_selection_labels_v4
         WHERE label_schema_version='canonical-strategy-selection-label-v4'
           AND (
             reference_contract_version <> 'selection-reference-snapshot-v3'
             OR entry_date <= signal_date
             OR exit_date < entry_date
             OR outcome_known_date <> exit_date
           )
    """, []),
...
def run_audit(query_fn: QueryFn) -> dict[str, Any]:
    ...
    for name, sql, params in CHECKS:
        ...
        violations = int((rows[0] if rows else {}).get("violations") or 0)
        checks.append({"name": name, "violations": violations, "status": "PASS" if violations == 0 else "FAIL"})
