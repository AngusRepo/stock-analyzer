...
export async function updateSchedulerExecutionTicket(
  ...
  const result = await db.prepare(`
    UPDATE scheduler_execution_tickets_v1
       SET status=?, status_authority=?, last_summary=?, last_error=?,
           ...
           END,
           updated_at=CURRENT_TIMESTAMP
     WHERE ticket_id=? AND run_id=? AND status IN (${placeholders})${canonicalGuard}
  `).bind(
    input.status,
    input.authority,
    input.summary ?? null,
  ...
  if (Number(result.meta?.changes ?? 0) !== 1 && (input.expectedPipelineCanonicalRunId || row.status !== input.status)) {
    throw new Error(
