...
const JOB_DEFS: JobDef[] = [
  ...
  { id: 'finlab-v4-backfill', name: 'FinLab V4 Backfill', schedule: 'Weekdays 18:30', cron: '30 10 * * 1-5', group: 'daily' },
...
export async function getSchedulerStatus(env: Bindings) {
  ...
  const jobs = await Promise.all(JOB_DEFS.map(async (def) => {
    ...
    return {
      ...
      lastStatus,
      status: lastStatus,
      run_id: lastLog?.run_id ?? null,
      run_date: lastLog?.run_date ?? null,
