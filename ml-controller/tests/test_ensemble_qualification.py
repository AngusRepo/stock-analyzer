...
def test_zero_signal_bundle_can_atomically_promote_and_stay_serving(monkeypatch):
    ...
    assert result["can_promote"] and result["readback_verified"]
    assert result["status"] == "ok"
    assert result["promotion_scope"] == "ranking"
    assert result["qualifications"]["directional"]["decision"] == "BLOCKED"
    serving = registry.load_active8_ensemble_serving_bundle()
    assert serving["status"] == "production" and serving["production_effect"] is True
    assert serving["blockers"] == []
    assert serving["qualifications"]["ranking"]["decision"] == "PASS"
