...
const materializeStart = adminWrite.indexOf("adminWriteRoutes.post('/api/admin/strategy/decision-log/materialize'")
const materializeEnd = adminWrite.indexOf("adminWriteRoutes.post('/api/admin/strategy/evidence-v5/rebuild'", materializeStart)
const materializeBlock = adminWrite.slice(materializeStart, materializeEnd)
assert.match(materializeBlock, /candidateReferenceDb: learningDb/)
assert.match(materializeBlock, /canonicalProducerRunId/)
assert.match(materializeBlock, /loadCanonicalScreenerRunIds/)
...
assert.match(cronGcp, /await env\.UPDATE_QUEUE\.send/)
assert.match(cronGcp, /scheduledTask: 's12-smcvwap-calibration'/)
assert.doesNotMatch(cronGcp, /runS12TwCalibration\(/)
assert.doesNotMatch(manual, /runS12TwCalibration\(/)
assert.match(manual, /s12 calibration requires durable queue execution/)
assert.match(adminTriggerRoutes, /task === 's12-smcvwap-calibration' && syncMode/)
assert.match(adminTriggerRoutes, /sync bypass is forbidden/)
