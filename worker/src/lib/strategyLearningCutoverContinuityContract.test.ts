...
test('continuity repair is bounded and cannot promote', () => {
  ...
  assert.match(repair, /strategy_router_score=COALESCE\(strategy_router_score, \?\)/)
  assert.match(repair, /strategy_learning_cutover_continuity_route_conflict/)
  assert.match(repair, /assertLegacyTargetParity\(legacyRouteRows, targetRowsAfter, true\)/)
