...
const packet = {
  ...
  sample_audit: { oof_max_date: '2026-08-18', sample_count: 7744, date_count: 24 },
  oos_metrics: { date_mean_cross_section_corr_lcb90: -0.04252319,
    ...
    date_mean_top_quintile_return_lcb90: -0.02895655 },
...
const evaluated = adaptExpectedReturnShadow({ ...row, validation_packet_json: JSON.stringify({
  ...packet, sample_audit: { ...packet.sample_audit, oof_max_date: '2026-09-03' },
}) })
...
const explicit = adaptExpectedReturnShadow({ ...row, validation_packet_json: JSON.stringify({
  ...packet, sample_audit: {}, monitoring_policy: { validation_scope: 'base_cohort_offline_validation' },
}) })
