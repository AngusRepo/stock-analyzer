...
$certDir = Join-Path $Root ".tmp\shioaji-official-api-test"
...
try {
  ...
  $args = @((Join-Path $Root "tools\shioaji_official_api_test.py"))
  if ($PlaceStockOrder) {
    $args += "--place-stock-order"
  }
  & $PythonPath @args
  exit $LASTEXITCODE
} finally {
