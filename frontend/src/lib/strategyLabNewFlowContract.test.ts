...
assert(page.includes("if (lane.key === 'candidate') onOpenAtomicV7(key)") && page.includes('點擊查看此策略 Atomic V7 全門檻 actual／target／pass'), 'Clicking a Candidate card must open its Atomic V7 modal')
...
assert(page.includes('Active 正式 contribution 分配') && page.includes('raw score = max(0, rolling date-return mean)') && page.includes('正式政策版本落後'), 'Strategy Lab must disclose the formal allocation formula and flag a stale persisted policy')
