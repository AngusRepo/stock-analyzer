...
assert(registry.includes("declaredState?.owner_state === 'learned_champion'"))
assert(registry.includes("owner_state: declaredState?.owner_state ?? 'safe_abstention'"))
assert(registry.includes('await db.batch(statements)'))
assert(registry.includes('expected_return_registry_pointer_batch_incomplete'))
assert(registry.includes('verifyNavPromotionEvidence(db'))
assert(registry.includes('navJournalFrontierGuard(db'))
...
assert(registry.includes('expectedReturnOfflineAdmissionBlockers(input.owner'))
...
assert(registry.includes('date_eligibility_json'))
assert(registry.includes("row.reason === 'missing_native_pit_components'"))
assert(registry.includes('isExpectedOofCurrentCloseCovered'))
assert(registry.includes('oof_not_evaluable_dates: oofNotEvaluableDates'))
...
assert(servingState.includes("pointer.owner_state === 'safe_abstention'"))
assert(servingState.includes('abstention_artifact_deprecated'))
...
assert(
  promoteBody.indexOf('pointerCommit = await commitExpectedReturnChampion')
    < promoteBody.indexOf('snapshot = await projectCommittedConfig(commitIdentity, pointerCommit'),
)
assert(promoteBody.includes('const snapshot = await setTradingConfig(c.env.KV, hydrated.config'))
assert(promoteBody.includes("const stored = await c.env.KV.get('trading:config', 'json')"))
assert(promoteBody.includes('already_committed: true'))
...
assert(!readinessBody.includes('runOpbArmPriorRefresh'), 'readiness cannot generate OPB candidates')
assert(!readinessBody.includes("logSchedulerResult(env.KV, 'opb-arm-prior-refresh'"), 'inspection cannot overwrite event task receipt as skipped')
