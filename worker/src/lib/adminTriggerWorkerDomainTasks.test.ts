...
assert.match(source, /run_scope: runScope/)
assert.match(source, /logSchedulerResult\(c\.env\.KV, 'strategy-learning'/)
assert.match(source, /logSchedulerResult\(c\.env\.KV, 'post-verify-chain'/)
assert.match(source, /logSchedulerResult\(c\.env\.KV, 'evening-chain'/)
...
assert.match(source, /target === 'superseded_pending_events'[\s\S]*paper_shadow_backfill_active/)
