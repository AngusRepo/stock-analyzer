...
assert(standalone.includes("['weekly', 'pipeline_chain', 'daily', 'intraday', 'monthly']") && standaloneCss.includes("[aria-label='Weekly operations'] { grid-column: span 4;") && standaloneCss.includes("[aria-label='Pipeline support'] { grid-column: span 2;") && standaloneCss.includes("[aria-label='Daily operations'] { grid-column: 1 / -1;") && standaloneCss.includes("[aria-label='Daily operations'] .obs-standalone__job-grid") && standaloneCss.includes('grid-template-columns: repeat(4, minmax(0, 1fr));'), 'standalone desktop top row must stay Weekly 4 / Pipeline 2 / Daily 4')
