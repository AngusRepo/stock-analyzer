...
const postRouteOverlays = fs.readFileSync('src/lib/screenerPostRouteOverlays.ts', 'utf8')
const postOverlaySeed = fs.readFileSync('src/lib/screenerPostOverlaySeed.ts', 'utf8')
const atomicStrategyShadow = fs.readFileSync('src/lib/atomicStrategyShadow.ts', 'utf8')
...
const pitResidualShadow = fs.readFileSync('src/lib/pitResidualShadow.ts', 'utf8')
...

{
  ...
  assert(marketScreener.includes('await buildLayer1WithAtomicSource({')
    && marketScreener.includes('universe: strategySourceUniverse as any')
    && marketScreener.includes('const layer1BreadthPlan = atomicSourceCapture.plan')
    && atomicStrategyShadow.includes('const plan = buildLayer1StrategyBreadthPlan(frozen.universe, frozen.specs, frozen.options)'),
    'full-universe source capture must still compute formal breadth through the original L1.5 owner')
  ...
  assert(marketScreener.includes("stage: 'pit_residual_momentum_shadow'"), 'screener must record the PIT residual counterfactual stage')
  assert(!marketScreener.includes("stage: 'rrg_overlay'"), 'screener must retire the RRG overlay stage')
  assert(marketScreener.includes('scoreBefore: candidate.score') && marketScreener.includes('scoreAfter: candidate.score'), 'PIT residual shadow must not mutate production score')
  assert(marketScreener.includes('candidateSetMutationAllowed: PIT_RESIDUAL_CANDIDATE_SET_MUTATION_ALLOWED'), 'PIT residual shadow must expose no candidate-set authority')
  assert(pitResidualShadow.includes("PIT_RESIDUAL_SHADOW_APPLICATION_MODE = 'shadow_pre_l2_candidate_rank_feature_only'"), 'PIT residual shadow must declare its research-only application mode')
  assert(pitResidualShadow.includes('PIT_RESIDUAL_SHADOW_WEIGHT = 0.10'), 'PIT residual shadow must keep the adjudicated 10% weight')
  assert(pitResidualShadow.includes('PIT_RESIDUAL_CANDIDATE_SET_MUTATION_ALLOWED = false'), 'PIT residual shadow must forbid candidate-set mutation')
  assert(pitResidualShadow.includes("AND decision_effect = 'none'"), 'PIT residual shadow must read only non-authoritative rows')
  assert(pitResidualShadow.includes('WHERE signal_date <= ?'), 'PIT residual shadow must use point-in-time rows as of the screener date')
  assert(!pendingBuyOrchestrator.includes('loadRrgTaxonomyTags'), 'pending-buy must not load RRG taxonomy authority')
  ...
  assert(marketScreener.includes('for (const event of applyScreenerBuzz(scored, overlayEligibleSymbols, {')
    ...
    && postRouteOverlays.includes("stage: 'buzz_evidence'")
    && postRouteOverlays.includes("reasonCode: 'weighted_keyword_evidence'"),
    'actual screener must publish weighted buzz evidence from its shared post-route owner')
  ...
  assert(!pendingBuyOrchestrator.includes('themeUniverse'), 'pending-buy must not rebuild a retired RRG theme universe')
  assert(!pendingBuyOrchestrator.includes('classification: quadrant.classification'), 'pending-buy logs must not expose retired RRG classification authority')
  assert(!pendingBuyOrchestrator.includes('RRG_UNMAPPED_NEUTRAL'), 'pending-buy must not emit retired RRG gate audit reasons')
  ...
  assert(screenerFunnelEvidence.includes('pit_residual_momentum_shadow'), 'funnel evidence summary must include PIT residual shadow evidence')
  assert(screenerFunnelEvidence.includes('layer0_universe_features_summary_v2'), 'funnel evidence summary must expose stable L0/L0.5 liquidity-capacity schema')
  ...
  assert(screenerFunnelEvidence.includes('strategy_portfolio_exact_metrics_with_structural_diversity_v2'), 'L1.25 FinLab portfolio intelligence must document FinLab-style strategy-as-asset method')
  ...
  assert(screenerFunnelEvidence.includes('deterministic_continuous_strategy_policy_router_v2'), 'L1.5 router summary must expose deterministic policy router method')
  ...
  assert(screenerFunnelEvidence.includes('continuous_weight_full_universe_no_candidate_veto'), 'L1.5 router summary must expose continuous full-universe no-veto policy')
  ...
  assert(recommendationContext.includes('l05_hard_gate_summary_v2'), 'recommendation context must expose the L0.5 liquidity-capacity schema')
  assert(recommendationContext.includes('exclude_untradable_untrusted_or_execution_infeasible_only_not_alpha_ranker'), 'L0.5 hard gate must document that it is not an alpha ranker')
  assert(recommendationContext.includes('tradeability_data_trust_liquidity_capacity_pending_buy'), 'L0.5 hard gate must document tradability/data trust/liquidity/pending-buy scope')
...

{
  ...
  assert(marketScreener.includes('assertCanonicalL15SeedIdentity({')
    && postOverlaySeed.includes('l15_canonical_seed_identity_mismatch'), 'L1/L1.5 identity mismatch must fail closed through the shared identity owner')
  ...
  assert(marketScreener.includes('applyScreenerRecentSessionSafety(finalCandidates, recentRows, postL15SafetyExcludedSymbols)')
    && marketScreener.includes('for (const event of events) pushFunnelItem(funnelItems, event)')
    && postRouteOverlays.includes("stage: 'l1_post_route_safety_gate'"), 'post-route safety exclusions must be written to funnel evidence')
  ...
  assert(marketScreener.includes('l15_route_rank: routeOwner.rank'), 'canonical L1 seed must bind the exact L1.5 route rank without an O(n²) lookup')
