...
export async function getLatestPrice(db: D1Database, symbol: string): Promise<number | null> {
  const row = await db.prepare(`
    SELECT
      CASE
        WHEN sp.open IS NULL AND sp.avg_price IS NOT NULL THEN sp.avg_price
        WHEN UPPER(COALESCE(s.market, '')) IN ('EMERGING', 'ROTC') THEN COALESCE(sp.avg_price, sp.close)
        ELSE COALESCE(sp.close, sp.avg_price)
      END as price
