...
const adminTriggerRoutes = createAdminTriggerRoutes({
  buildTaskMap: (c: any) => buildAdminTriggerTaskMap(c, {
    ...
    runWeeklyBacktest: (runDate?: string) => runWeeklyBacktestWorkflow(c.env, runDate),
    runWeeklyMonteCarlo: (runDate?: string) => runWeeklyMonteCarloWorkflow(c.env, runDate),
    runWeeklyPBO: (runDate?: string) => runWeeklyPboWorkflow(c.env, runDate),
