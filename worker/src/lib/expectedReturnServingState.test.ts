...
assert.equal(fusionPrimary.overlay_status, 'applied')
...
assert.equal(legacyV11StillServes.overlay_status, 'abstained')
...
assert.equal(hybridRejected.expected_return_owner, 'allocator_ev_fusion')
assert.equal(hybridRejected.overlay_status, 'abstained')
...
assert.equal(l4Fallback.expected_return_owner, 'allocator_ev_fusion')
assert.equal(l4Fallback.action_gate, 'expected_return_owner')
assert.equal(l4Fallback.overlay_status, 'abstained')
assert.equal(l4Fallback.artifacts.allocator_ev_fusion.artifact_state, 'candidate_not_ready')


const overlayPointerWarning = resolveExpectedReturnServingState({
  ensemble_v2: { l4AlphaEv: validL4() },
}, {
  alerts: [
    'allocator_ev_fusion:champion_pointer_missing',
    'l4_alpha_ev:unrelated_integrity_failure',
  ],
})
assert.deepEqual(overlayPointerWarning.hard_alerts, ['l4_alpha_ev:unrelated_integrity_failure'])
assert(overlayPointerWarning.warnings.includes('overlay_warning:allocator_ev_fusion:champion_pointer_missing'))
assert.equal(overlayPointerWarning.overlay_status, 'abstained')

const noL4 = resolveExpectedReturnServingState({}, { alerts: ['allocator_ev_fusion:champion_pointer_missing'] })
assert.deepEqual(noL4.hard_alerts, ['allocator_ev_fusion:champion_pointer_missing'])
