...
assert(healthBucket.includes("return 'performance_cooldown'"), 'Active with zero formal contribution must be classified as performance cooldown')
assert(page.includes('gate.allocation_eligible === true && formalWeight > 0'), 'Pending-buy health requires both the allocation gate and a positive formal weight')
...
assert(api.includes('base_lifecycle_recommendations') && page.includes('formalPolicy.base_policy_version === formalBasePolicyVersion') && page.includes('formalPolicy.base_policy_as_of_date === formalBasePolicyAsOfDate') && page.includes('formalPolicy.base_policy_run_id'), 'Cooldown reasons must come from the exact persisted formal production base policy lineage')
assert(page.includes('formalLifecycleRecommendations[row.id]') && page.includes('正式 policy 降溫原因') && page.includes('為什麼被排入績效降溫') && page.includes('formalCooldownReasons.map'), 'Active cooldown cards and inspector must expose every formal policy reason')
