...
assert.match(screener, /strategyWeights: evaluationStrategyWeights/)
assert.match(screener, /runtimeStrategyAllocationWeights = runtimeStrategyWeightResolution\.allocationWeights/)
assert.match(screener, /hasPositiveStrategyAllocation/)
...
assert.match(reference, /selection_evidence_staging_coverage_mismatch/)
...
assert.match(canonicalLabels, /r\.feature_contract_version IN \(\?, \?\)/)
assert.match(canonicalLabels, /LEFT JOIN canonical_selection_labels_v4 l/)
assert.match(canonicalLabels, /l\.entry_date IS NOT h\.entry_date/)
assert.match(canonicalLabels, /JSON\.stringify\(dirtyGroups\)/)
...
assert.match(canonicalLabels, /SELECTION_REFERENCE_MATURE_COMPATIBLE_CONTRACT_VERSIONS/)
...
assert.match(canonicalLabels, /l\.adjustment_source IS NOT \?/)
assert.match(canonicalLabels, /CANONICAL_SELECTION_ADJUSTMENT_SOURCE, row\.reference\.feature_contract_version/)
