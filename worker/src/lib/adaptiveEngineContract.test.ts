...
void (async () => {
  ...
  try {
    ...
    assert(modelAccuracyCalls.length >= 3, 'adaptive update should query active-8 model_accuracy evidence')
    assert(modelAccuracyCalls.every((call) => call.sql.includes('model_name IN')), 'model_accuracy evidence queries must be scoped to active-8 models')
    const allBinds = modelAccuracyCalls.flatMap((call) => call.binds)
    assert(allBinds.includes('LightGBM') && allBinds.includes('TabM') && allBinds.includes('iTransformer') && !allBinds.includes('TimesFM'), 'active-8 model binds must include tree, TabM, sequence and exclude TimesFM sidecar')
    assert(!allBinds.includes('CatBoost'), 'active-8 adaptive evidence must not bind retired CatBoost')
