...
def archived_predictions(start_date, end_date, *, date_column='prediction_date', model_name=None,
    ...
    with history_keys() as remember:
        while True:
            manifests=ops(f"""SELECT a.artifact_id,a.domain,a.schema_version,a.checksum,a.row_count,
               ...
               WHERE a.domain='retention_learning_lineage_v1_predictions'
                 ...
                 AND COALESCE(CASE WHEN json_valid(a.metadata_json) THEN json_extract(a.metadata_json,'$.{coverage}_end') END,'9999-12-31')>=?
                 AND COALESCE(CASE WHEN json_valid(a.metadata_json) THEN json_extract(a.metadata_json,'$.{coverage}_start') END,'0000-01-01')<=?
               ORDER BY a.artifact_id LIMIT 100""",[last,start_date,end_date])
            ...
            for manifest in manifests:
                ...
                for row in relevant:
                    ...
                    if not remember(key,encoded):continue
                    yield clean
