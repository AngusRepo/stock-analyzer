...
assert(workflow.includes("--format '{{ json (index .SBOM \"linux/amd64\").SPDX }}'"), 'P9 workflow must read back the registry SBOM for the deployed platform')
assert(workflow.includes("--format '{{ json (index .Provenance \"linux/amd64\").SLSA }}'"), 'P9 workflow must read back registry provenance for the deployed platform')
