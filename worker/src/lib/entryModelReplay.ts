...
export async function ensureEntryModelReplayReportTable(db: D1Database): Promise<void> {
  await db.prepare(`
...
export async function persistEntryModelReplayReport(
  db: D1Database,
  report: EntryModelReplayReport,
): Promise<{ report_id: string; inserted: boolean }> {
  await ensureEntryModelReplayReportTable(db)
  const reportId = [
    'entry_model_replay',
    report.options.startDate,
    report.options.endDate,
    Date.now().toString(36),
  ].join(':')
  await db.prepare(`
    INSERT INTO entry_model_replay_reports (
      report_id,
      version,
      start_date,
      end_date,
  ...
  `).bind(
    ...
    report.options.endDate,
    report.loadedCases,
    report.promotionGate.decision,
    report.promotionGate.passed ? 1 : 0,
    JSON.stringify(report.promotionGate.failedGates),
    JSON.stringify(report.summary),
    JSON.stringify(report.promotionGate),
    JSON.stringify(report),
  ).run()
  return { report_id: reportId, inserted: true }
}

export async function getLatestEntryModelReplayReport(
  db: D1Database,
  ...
  const row = await db.prepare(`
    SELECT report_id,
           ...
           report_json,
           created_at
      FROM entry_model_replay_reports
  ...
  return {
    ...
    failed_gates: parse(row.failed_gates_json),
    summary: parse(row.summary_json),
    promotion_gate: parse(row.promotion_gate_json),
