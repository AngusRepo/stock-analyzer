...
_ARTIFACT_CACHE: dict[tuple[str, ...], "GraphSAGEArtifact"] = {}
    ...
    if str(entry.get("serving_owner") or "") == "frozen_pipeline_modal_serving_manifest":
        for key in ("gcs_path", "metadata_path", "checksum", "serving_artifact_id"):
            if not str(resolved.get(key) or "").strip():
                raise RuntimeError(f"GNN frozen artifact identity missing {key}")
    return resolved
...
def load_graphsage_artifact(pool: dict | None = None) -> GraphSAGEArtifact:
    ...
    if frozen_owner and not metadata_model:
        raise RuntimeError("GNN registry metadata model missing")
    if frozen_owner and not metadata_version:
        raise RuntimeError("GNN registry metadata version missing")
    if metadata_model and metadata_model != MODEL_NAME:
        raise RuntimeError("GNN registry metadata model mismatch")
    if metadata_version and metadata_version != version:
        raise RuntimeError("GNN registry metadata version mismatch")
    if metadata_artifact_id and artifact_id and metadata_artifact_id != artifact_id:
        raise RuntimeError("GNN registry metadata artifact id mismatch")
    if expected_checksum and metadata_checksum != expected_checksum:
        raise RuntimeError("GNN registry/metadata checksum mismatch")
    try:
