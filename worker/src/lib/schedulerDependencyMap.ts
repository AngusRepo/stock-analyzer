...
export const SCHEDULER_DEPENDENCY_MAP: Record<string, SchedulerDependencySpec> = {
  ...
  'weekly-cleanup': {
    ...
    currentFunction: 'Runs local cleanup, lifecycle check, and weekly local maintenance.',
    replacementOwner: 'manual maintenance + model registry/promotion controller',
    upstream: [],
    downstream: ['model-registry-maintenance'],
    requiredBeforeDisable: [
      'lifecycle check must be owned by model registry/promotion controller',
    ...
    operatorRisk: 'medium',
    recommendation: 'Convert to manual/monthly maintenance; remove lifecycle side effects from this job.',
  },
  'optuna-queue': {
    task: 'optuna-queue',
    owner: 'gcp_scheduler',
