...
assert.match(source, /activeDataDomainShadowBackfillRunId\(c\.env\.KV, 'ops'\)/)
assert.match(source, /skipped=ops_shadow_backfill_active/)
...
assert.match(source, /CUTOVER_PROBE_DOMAINS = new Set<DataDomain>\(DATA_DOMAINS\)/)
assert.match(source, /shadowDatabaseForDataDomain\(c\.env, requestedDomain\)/)
assert.match(source, /installDataDomainWriterEpochTriggers\(c\.env\.DB, requestedDomain\)/)
...
assert.match(updateOrchestrator, /s12ReplayLifecycleMutationAllowed\(replayScope, replayPersist\)/)
...
assert.match(source, /manual-l0-replay-/)
assert.match(source, /replayEndOffset: offset \+ limit/)
assert.doesNotMatch(source, /runS12HistoricalReplayForDate\(c\.env/)
