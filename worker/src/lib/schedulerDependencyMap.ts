...
export const SCHEDULER_DEPENDENCY_MAP: Record<string, SchedulerDependencySpec> = {
  ...
  'active8-oof-weekly': {
    ...
    owner: 'gcp_scheduler',
    consolidationClass: 'keep_scheduler',
    currentFunction: 'Creates the deterministic weekly purged-OOF cohort and its immutable lineage.',
    upstream: ['verified-outcomes', 'dataset-snapshot-export'],
    downstream: ['active8-oof-daily', 'active8-oof-monthly'],
    requiredBeforeDisable: [],
    operatorRisk: 'high',
    recommendation: 'Keep as the weekly cohort owner; it is not the daily materializer or monthly release.',
  },
  'active8-oof-monthly': {
    task: 'active8-oof-monthly',
    owner: 'gcp_scheduler',
    consolidationClass: 'keep_scheduler',
    currentFunction: 'Runs the monthly full-fit release, bundle validation, and atomic promotion lifecycle.',
    upstream: ['active8-oof-weekly', 'monthly-optuna'],
    downstream: ['model-registry', 'serving-bundle'],
    requiredBeforeDisable: [],
    operatorRisk: 'high',
    recommendation: 'Keep as the only monthly Active-8 release owner.',
  },
  'evening-chain': {
    task: 'evening-chain',
    owner: 'gcp_scheduler',
