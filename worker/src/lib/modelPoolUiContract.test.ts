...
assert(page.includes('影子挑戰者'), 'Model Pool must distinguish shadow challengers from active alpha models')
...
assert(page.includes('Model Upgrade Tracks /'), 'Model Pool must show the full P7 model upgrade track, not only model-family challengers')
...
assert(page.includes('track_only'), 'Model Pool must render non-experiment model-upgrade lanes as track_only')
assert(page.includes('這個軌道不走 Strategy Lab experiment registry'), 'Track-only lanes must not look like registry evidence lanes')
...
assert(page.includes('experiment_missing') && page.includes('research experiment registry missing'), 'Model-upgrade experiment lanes must clearly show missing research experiment evidence instead of fake trained state')
...
for (const id of ['Chronos2ZeroShot', 'Chronos2LoRA', 'ResidualMLP', 'GNN', 'TabM', 'iTransformer', 'TimesFM', 'GAOptimizer', 'KalmanFilter', 'MarkovSwitching']) {
