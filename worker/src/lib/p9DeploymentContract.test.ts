...
const mlRequirements = fs.readFileSync('../ml-service/requirements.txt', 'utf8')
...
assert(/^ray==2\.56\.0$/m.test(mlRequirements), 'ML runtime must retain the pinned Ray dependency required by NeuralForecast BaseAuto imports')
assert(mlDockerfile.includes('18e8dc31664dea28d607a015184b3d4543f586c9dd41d16330e73e80b3a11e43'), 'ML image must verify the reviewed Ray Java artifact before removal')
assert(mlDockerfile.includes("jar.unlink()"), 'ML image must remove the unused vulnerable Ray Java API artifact')
assert(mlDockerfile.includes("assert not Path(ray.__file__).with_name('jars').joinpath('ray_dist.jar').exists()"), 'ML image imports must fail closed if the Ray Java artifact remains')
