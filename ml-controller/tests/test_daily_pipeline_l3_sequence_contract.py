...
def test_daily_pipeline_calls_all_l3_sequence_predictors():
    ...
    assert "sequence_contract_points=sequence_contract_points" in source
    ...
    l3_body = source[
        source.index("async def node_ml_predict"):
        source.index("def _timesfm_l175_registry_release_policy")
    ]
    assert "modal_client.timesfm_batch_predict" not in l3_body
    assert "timesfm_universal_predict" not in l3_body
...
def test_l2_tree_gate_node_is_not_in_daily_hot_path():
    source = (ROOT / "graphs" / "daily_pipeline_v2.py").read_text(encoding="utf-8")

    assert "async def node_l2_cheap_ml_predict" not in source
    assert "node_l2_core_gate" not in source
    assert "modal_client.l2_tree_batch_predict" not in source
    assert "predict_l2_tree_batch" not in source


def test_l2_routes_through_timesfm_sidecar_before_l3():
    source = (ROOT / "graphs" / "daily_pipeline_v2.py").read_text(encoding="utf-8")
...
def test_timesfm_l175_release_retrain_path_materializes_l2_features():
    ...
    assert "stock_meta=getattr(req, \"stock_meta\", None)" in batch_prediction
    assert "predict_l2_tree_batch" not in batch_prediction
