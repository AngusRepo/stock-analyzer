...
assert(workbench.includes('return selectionCandidate(row) ?? row?.serving_release_artifact ?? null'), 'Fleet evidence should use a selected candidate or the canonical serving artifact, never the latest rejected retrain')
